...
backgrond_img=pygame.image.load(os.path.join(folder,'img','background.png')).convert()
...
def draw_text(surt,text,size,x,y):
    font=pygame.font.Font(font_name,size)
    text_surface=font.render(text,True,WHITE)
    text_rect=text_surface.get_rect()
    text_rect.centerx=x
    text_rect.top=y
    surt.blit(text_surface,text_rect)
...
while running:
    ...
    for hit in hits_rockandbullet:
        ...
        expl=baozha(hit.rect.center,'lg')
    ...
    for hit in hits_playerandrock:
        ...
        new_rock()
        expl=baozha(hit.rect.center,'sm')
        all_sprites.add(expl)
        if player.health<=0:
            death_expl=baozha(player.rect.center,'player')
